            ...
            <View style={tailwind`pb-5  w-full px-4 bg-white pt-4`}>
                ...
                <View style={tailwind`mb-5`}>
                    {allCartItems?.map(item => (
                        <OrderItem key={item.resName} name={item.resName} value={`R${item?.foods.reduce((total, item) => total + item.price, 0).toFixed(1)} • (${item?.foods?.length})`} />
                    ))}
                    <OrderItem name="Total price" value={`R${totalPrice}`} total />
                </View>
