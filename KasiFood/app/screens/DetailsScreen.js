				...
				<ScrollView showsVerticalScrollIndicator={false} style={tailwind`z-20`}>
					<View style={styles.content}>
						<View style={styles.header}>
							...
							<TouchableOpacity onPress={() => setMapActive((e) => !e)}>
								<Entypo
									name='location'
									size={24}
									color={`${mapActive ? colors.primary : '#00bfbc'}`}
								/>
							</TouchableOpacity>
						</View>
						<View
						...
						<View style={tailwind`mt-3`}>
							<Text
								style={[
									tailwind`text-gray-800 font-bold border-b w-1/3 mb-2 pb-1`,
									{ borderBottomColor: colors.primary, fontSize: 17 },
								]}
							>
								Categories
							</Text>
							{categories.map(({ title }, index) => (
								<Text key={index} style={tailwind`text-xs text-gray-700`}>
									<Text style={{ color: colors.primary }}>•</Text> {title}
								</Text>
							))}
						</View>
						{/* MenuItems */}
